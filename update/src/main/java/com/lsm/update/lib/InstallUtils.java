...
import com.lsm.update.lib.utils.DownloadFileUtils;
import com.lsm.update.lib.utils.ICallback;
import com.lsm.update.lib.utils.ResultRequest;
import com.lsm.update.lib.utils.Utils;
...
public class InstallUtils {
    ...
    public void startDownload() {
        ...
        if (TextUtils.isEmpty(filePath)) {
            filePath = Utils.getCachePath(mContext) + "/update.apk";
        }
        //文件权限处理
        Utils.changeApkFileMode(new File(filePath));
        //下载
        DownloadFileUtils.with()
                .downloadPath(filePath)
                .url(httpUrl)
                .tag(InstallUtils.class)
                .execute(new BaseFileProgressCallback() {
    ...
    public static void installAPK(Activity context, String filePath, final InstallCallBack callBack) {
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                ...
                apkUri = UpdateApkFileProvider.getUriForFile(context, authority, apkFile);
            } else {
                apkUri = Uri.fromFile(apkFile);
            }
            intent.setDataAndType(apkUri, "application/vnd.android.package-archive");
            new ResultRequest(context).startForResult(intent, new ICallback() {
                @Override
    ...
    public static void openInstallPermissionSetting(Activity activity, final InstallPermissionCallBack installPermissionCallBack) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            new ResultRequest(activity).startForResult(intent, new ICallback() {
                @Override
                public void onActivityResult(int resultCode, Intent data) {
                    Log.i(TAG, "onActivityResult:" + resultCode);
